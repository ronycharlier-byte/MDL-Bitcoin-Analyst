...
| G-003 | Fundamental coverage is partial: live API runs may include real point-in-time values for ETF flows, funding rate, open interest, hash rate, exchange reserves, stablecoin supply, DXY, US rates and Nasdaq when sources respond; liquidations are real only when observed from Bitget WebSocket; missing fields remain absent. | data_quality | high | partial_real_absent |
| G-004 | The latest confidence score is 76/100 in the current latest_report.md export. | model_output | high | inferred |
